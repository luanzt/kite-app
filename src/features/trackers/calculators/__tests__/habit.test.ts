...
const habit: Tracker = {
  ...
  period: 'daily', repeatDays: [0,1,2,3,4,5,6], routine: 'any', reminderTime: null,
  createdAt: '2026-06-01T00:00:00Z', archived: false,
};
const done = (date: string): Entry => ({ id: date, trackerId: 'h1', date, value: 1, note: null });
const log = (date: string, value: number): Entry => ({ id: `${date}-${value}`, trackerId: 'h1', date, value, note: null });
...
describe('calculateHabit', () => {
  ...
  test('a single log of the full Goal value also counts the day as done', () => {
    const goal3 = { ...habit, targetValue: 3 };
    const p = calculateHabit(goal3, [log('2026-06-13', 3), log('2026-06-14', 3)], '2026-06-14');
    expect(p.streak).toBe(2);
  });
