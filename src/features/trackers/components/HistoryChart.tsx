...
export function HistoryChart({
  ...
  const cumulative = tracker?.type === 'target' && tracker?.accumulation !== 'latest';
  ...
  const pace = showPace
    ? actual.map((_, i) => start + (goal - start) * (i / Math.max(1, actual.length - 1)))
    : [];

  const max = Math.max(...actual, ...(showPace ? pace : []), 1) * 1.1;
  const X = (i: number) => pad + (i / Math.max(1, actual.length - 1)) * (W - pad * 2);
  const Y = (v: number) => H - pad - (v / max) * (H - pad * 2);

  const lineOf = (series: number[]) =>
    series.map((v, i) => `${i ? 'L' : 'M'}${X(i).toFixed(1)} ${Y(v).toFixed(1)}`).join(' ');
  const actualLine = lineOf(actual);
  const area = `${actualLine} L${X(actual.length - 1).toFixed(1)} ${H - pad} L${X(0).toFixed(1)} ${H - pad} Z`;
    ...
    <View className="rounded-lg-k bg-surface border border-line px-s3 pt-s4 pb-s2">
      ...
      <View className="flex-row gap-s4 px-s2 pt-s2">
        <View className="flex-row items-center gap-s1">
          <View className="w-3.5 h-0.5 rounded-full" style={{ backgroundColor: col }} />
