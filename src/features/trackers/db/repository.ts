...
export function listTrackers(): Tracker[] {
  const res = getDb().executeSync(`SELECT ${COLS} FROM trackers WHERE archived = 0 ORDER BY created_at DESC`);
  return (res.rows ?? []).map(rowToTracker);
}

export function getTracker(id: string): Tracker | null {
  const res = getDb().executeSync(`SELECT ${COLS} FROM trackers WHERE id = ?`, [id]);
