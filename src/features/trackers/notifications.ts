...
export async function cancelTrackerReminders(trackerId: string): Promise<void> {
  try {
    await Promise.all(
      ALL_DAYS.map(day => notifee.cancelTriggerNotification(reminderId(trackerId, day))),
    );
  } catch {
    // ignore
  }
 ...
 */
export async function scheduleTrackerReminders(tracker: Tracker): Promise<void> {
  ...
  const days = tracker.repeatDays && tracker.repeatDays.length > 0 ? tracker.repeatDays : ALL_DAYS;
