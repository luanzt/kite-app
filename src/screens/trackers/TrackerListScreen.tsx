...
export function TrackerListScreen() {
  ...
  const header = (
    ...
    >
      <Typography className="text-lg font-bold text-ink">{t('list.title')}</Typography>
          ...
          <View className="flex-row flex-wrap px-s5 gap-s3">
            {QUICK_STARTS.map(qs => (
              <Pressable
                ...
                style={{ width: '48%', paddingVertical: 13, paddingHorizontal: 14 }}
              >
                <Typography style={{ fontSize: 20 }}>{iconEmoji(qs.icon)}</Typography>
                <Typography className="flex-1 text-sm font-bold text-ink" numberOfLines={1}>
  ...
  return (
    <View className="flex-1 bg-bg">
      ...
      <Pressable
        ...
        style={{ right: 18, bottom: 18, width: 58, height: 58, borderRadius: 20 }}
      >
