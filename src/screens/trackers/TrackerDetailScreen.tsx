...
import { useTracker, useEntries, useMilestones, useLogEntry } from '@features/trackers/queries';
...
function Stat({
  ...
  return (
    <View className="flex-1 items-center rounded-lg-k border border-line bg-surface p-s4">
      ...
      <Typography className="text-xs font-bold uppercase text-ink-3" style={{ marginTop: 3 }}>
        {cap}
      </Typography>
    </View>
  );
...
export function TrackerDetailScreen({ route }: RootStackProps<'TrackerDetail'>) {
  ...
  const appbar = (
    ...
    >
      ...
      <Typography className="flex-1 text-center text-lg font-bold text-ink" numberOfLines={1}>
        {tracker.name}
      </Typography>
      <Pressable
        onPress={() => nav.navigate('TrackerForm', { trackerId: tracker.id, type: tracker.type })}
        className="items-center justify-center rounded-md-k border border-line bg-surface active:opacity-80"
        style={{ width: 40, height: 40 }}
      >
  ...
  const hero =
    tracker.type === 'habit' ? (
      <View className="m-s5 items-center rounded-xl-k border border-line bg-surface p-s5 shadow-md">
        <View className="items-center justify-center" style={{ width: 120, height: 120, marginVertical: 4 }}>
          <Ring fraction={p.successRate ?? 0} color={PACE_COLOR.on_track} size={120} strokeWidth={11} />
          <View className="absolute inset-0 items-center justify-center">
            <Typography className="font-extrabold text-ink" style={{ fontSize: 34, lineHeight: 36 }}>
              {`${successInt}%`}
            </Typography>
            <Typography className="text-xs font-bold text-ink-3">{t('detail.success')}</Typography>
          </View>
        </View>
      ...
      <View className="m-s5 rounded-xl-k border border-line bg-surface p-s5 shadow-md">
        <View className="flex-row items-start justify-between" style={{ marginBottom: 16 }}>
          <View className="flex-1">
            <Typography className="font-extrabold text-ink" style={{ fontSize: 48, lineHeight: 50 }}>
              {tracker.type === 'project' ? `${percentInt}%` : fmtVal(tracker, p.current)}
            </Typography>
        ...
        <PaceBar percent={p.percent} paceStatus={p.paceStatus} paceMarkerPercent={pp} height={16} />
        {pp != null ? (
          <View className="flex-row items-center justify-between" style={{ marginTop: 14 }}>
            ...
            {remain != null ? (
              <Typography className="text-xs text-ink-3">
                {`${remain} ${t('detail.days')} ${t('detail.remaining').toLowerCase()}`}
              </Typography>
            ) : null}
          </View>
        ) : null}
  ...
  const stats =
    tracker.type === 'habit' ? (
      <View className="flex-row gap-s3 px-s5">
        <Stat num={String(streak)} cap={t('detail.streak')} color={PACE_COLOR.on_track} />
      ...
      <View className="flex-row gap-s3 px-s5">
        ...
        <Stat num={`${doneMilestones}/${milestones.length}`} cap={t('detail.milestones')} />
        <Stat num={remain != null ? String(remain) : '∞'} cap={t('detail.days')} />
      </View>
    ) : (
      <View className="flex-row gap-s3 px-s5">
